...
function asegurarHojaEntradas(ss) {
  ...
  if (!hoja) {
    ...
    hoja.appendRow(['ID', 'SUCURSAL', 'NOMBRE', 'PRECIO', 'FECHA', 'OBS', 'TIMESTAMP']);
    hoja.getRange(1, 1, 1, 7).setFontWeight('bold').setBackground('#0d0d0d').setFontColor('#ffffff');
    hoja.setFrozenRows(1);
  }
  return hoja;
}
...
function obtenerEntradas(sucursalFiltro) {
  try {
    ...
    for (var i = 1; i < datos.length; i++) {
      ...
      entradas[suc].push({ id: id, sucursal: suc, nombre: nombre, precio: precio, fecha: fecha, obs: obs });
    }
...
function guardarEntrada(datos) {
  try {
    ...
    hoja.appendRow([nuevoId, sucursal, nombre, precio, fecha, obs, ts]);
